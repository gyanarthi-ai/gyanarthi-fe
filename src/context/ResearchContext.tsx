...
interface ResearchContextType {
  ...
  generateArticle: (query: string, format: IArticleFormat) => Promise<string>;
...
export const ResearchProvider = ({ children }: { children: ReactNode }) => {
  ...
  const updateResearch = (updates: Partial<Research>) => {
    setResearch((prev) =>
      prev ? { ...prev, ...updates } : ({ ...updates } as Research),
    );
  };
  ...
  const reviewResearch = async () => {
    ...
    try {
      const res = await axiosInstance.post(
        `/research/written_article/${research.id}/review`,
        {
          title: research.title,
          content: research.content,
        },
  ...
  const generateArticle = async (query: string, format: IArticleFormat) => {
    if (!research) return;
    try {
      const res = await axiosInstance.post(`research/generate/full`, {
